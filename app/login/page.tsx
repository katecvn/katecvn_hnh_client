...
const GoogleLoginPage = () => {
  ...
  useEffect(() => {
    const listener = (event: MessageEvent) => {
      console.log('📥 Nhận message:', event);
      console.log('📦 Dữ liệu nhận được:', event.data);

      // ✅ Lọc đúng định dạng message bạn gửi từ popup
      const isValid =
        typeof event.data === 'object' &&
        event.data !== null &&
        'type' in event.data &&
        typeof event.data.type === 'string' &&
        ['google-auth-code', 'google-auth-error'].includes(event.data.type);

      if (!isValid) {
        console.log('⏩ Message không hợp lệ hoặc không liên quan, bỏ qua.');
        return;
      }
      ...
      if (type === 'google-auth-code') {
        console.log('✅ Nhận mã code từ popup:', payload?.rawQuery);
        // Gọi API xác thực, lưu token tại đây
      }

      if (type === 'google-auth-error') {
        console.warn('❌ Nhận thông báo lỗi từ popup');
      }
    };
