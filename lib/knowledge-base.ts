...
export class KnowledgeBaseManager {
  ...
  private checkEmbeddingSupport(): boolean {
    ...
    if (!hasApiKey) {
      console.warn(
        'OpenAI API key not found. Knowledge Base will use keyword search fallback.'
      );
    }
      ...
      for (const doc of this.documents) {
        ...
        for (const chunk of chunks) {
          ...
          const keywordMatches = queryLower
            .split(' ')
            .filter(
              (word) => word.length > 2 && chunkLower.includes(word)
            ).length;
  ...
  async addDocument(document: KnowledgeDocument): Promise<void> {
    ...
    try {
      ...
      this.documents.push(document);
      console.log(
        `Added document without embedding (fallback): ${document.title}`
      );
    }
  }
  ...
  getStats() {
    return {
      ...
      documentsWithEmbeddings: this.documents.filter((doc) => doc.embedding)
        .length,
