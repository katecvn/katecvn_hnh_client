...
import { ArrowRight, Cpu, TrendingUp, Zap, Database, Shield, Code2 } from "lucide-react"
import { cn } from "@/lib/utils"
import { Reveal } from "./enhanced-animations"
import { NeonBorder, HolographicText, CyberButton } from "./tech-blue-animations"
    ...
    <Reveal direction="up" delay={delay}>
      ...
      >
        ...
        <CardHeader className="relative z-10">
          ...
          >
            <div className="group-hover:animate-bounce text-tech-blue-600">{icon}</div>
          </div>
          ...
          <CardDescription className="text-base leading-relaxed text-tech-blue-700">{description}</CardDescription>
        </CardHeader>
...
export function StatsCard({ value, label, icon, trend, delay = 0 }: StatsCardProps) {
  return (
    <Reveal direction="up" delay={delay}>
      <div className="text-center group hover-lift glass-tech rounded-lg p-6">
        <div className="relative inline-block mb-4">
          <div className="w-16 h-16 bg-gradient-to-r from-tech-blue-100 to-cyber-blue/20 rounded-full flex items-center justify-center mx-auto group-hover:scale-110 transition-transform duration-300 animate-tech-pulse">
            <div className="text-tech-blue-600 group-hover:animate-bounce">{icon}</div>
          </div>
          {trend && (
        ...
        <div className="text-tech-blue-600 group-hover:text-tech-blue-800 transition-colors">{label}</div>
...
export function FeatureCard({ title, description, icon, features, delay = 0 }: FeatureCardProps) {
  return (
    <Reveal direction="up" delay={delay}>
      <div className="text-center group glass-tech rounded-lg p-6 hover-lift">
        <div className="relative mb-6">
          ...
          >
            <div className="text-tech-blue-600 group-hover:animate-pulse">{icon}</div>
          </NeonBorder>
        </div>
...
export function TechProductCard({ title, description, image, badge, badgeColor, delay = 0 }: TechProductCardProps) {
  return (
    <Reveal direction="up" delay={delay}>
      <NeonBorder color="blue" className="group overflow-hidden transition-all duration-500 hover-lift bg-white">
        <div className="relative overflow-hidden">
          ...
          <div className="absolute top-4 left-4">
            <Badge className={cn("animate-tech-pulse", badgeColor)}>{badge}</Badge>
          </div>
        ...
        <CardHeader>
          ...
          <CardDescription className="text-base text-tech-blue-700">{description}</CardDescription>
        </CardHeader>
